...
#' @param max_iter Maximum number of iterations at each point on the lambda path
...
scope = function ( x, y, gamma = 8, lambda = NULL, nlambda = 100, lambda_min_ratio = 0.01, nfolds = 5, include_intercept = TRUE, return_full_beta = FALSE,
                   max_iter = 1000, early_stopping = ifelse(pshrink > 1, TRUE, FALSE), early_stopping_rounds = 20, early_stopping_criterion = "AIC", noise_variance = NULL, 
                   terminate_eps = 1e-7, silent = TRUE, only_cross_validate = FALSE, grid_safe = 10, block_order = NULL, fold_assignment = NULL) { 
  ...
  if ( nfolds > 1 ) {
    ...
    for ( k in 1:nfolds ) {
      ...
      cvsolution = core_scope(yfold, xlinearfold, xshrinkfold, block_order, k, gamma, (early_stopping_criterion == "AIC"), early_stopping_criterion, lambda, terminate_eps,
                              !include_intercept, max_iter, early_stopping, early_stopping_rounds,
                              silent, grid_safe )
      if ( k == 1 ) {
    ...
    fullsolution = core_scope(y, xlinear, xshrink, block_order, 2, gamma, (early_stopping_criterion == "AIC"), early_stopping_criterion, lambda, terminate_eps,
                              !include_intercept, max_iter, early_stopping, early_stopping_rounds,
                              silent, grid_safe )
    ...
    solution = core_scope(y, xlinear, xshrink, block_order, 1, gamma, (early_stopping_criterion == "AIC"), early_stopping_criterion, lambda, terminate_eps,
                          !include_intercept, max_iter, early_stopping, early_stopping_rounds,
                          silent, grid_safe )
